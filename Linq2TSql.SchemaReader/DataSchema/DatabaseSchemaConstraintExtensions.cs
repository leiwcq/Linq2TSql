    ...
    public static class DatabaseSchemaConstraintExtensions
    {
            ...
            string foreignTableSchemaOwner)
        {
            ...
            if (string.IsNullOrEmpty(foreignKeyName))
                //no fk name, so we'll invent one (it may be too long e.g. for Oracle)
            {
            ...
            var fkTable = string.IsNullOrEmpty(foreignTableSchemaOwner)
                ? table.DatabaseSchema.FindTableByName(foreignTableName)
                : table.DatabaseSchema.FindTableByName(foreignTableName, foreignTableSchemaOwner);
