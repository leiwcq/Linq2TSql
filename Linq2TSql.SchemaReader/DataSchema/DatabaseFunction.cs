...
namespace Linq2TSql.SchemaReader.DataSchema
{
    /// <summary>
    ...
    {
        [DebuggerBrowsable(DebuggerBrowsableState.Never)] private string _returnType;
        ...
        public void CheckArgumentsForReturnType()
        {
            ...
            DatabaseArgument returnArgument =
                Arguments.FirstOrDefault(argument => (argument.Out && string.IsNullOrEmpty(argument.Name)) ||
                                                     //MySql parameters may be marked "IN" so we look for ordinal and name
                                                     (argument.Name.Equals("RETURN_VALUE",
                                                         StringComparison.OrdinalIgnoreCase) && argument.Ordinal == 0));
            //couldn't find one
