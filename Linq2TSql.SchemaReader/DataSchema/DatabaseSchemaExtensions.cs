...
namespace Linq2TSql.SchemaReader.DataSchema
{
    ...
    public static class DatabaseSchemaExtensions
    {
        ...
        {
            if (databaseTable == null) throw new ArgumentNullException("databaseTable", "databaseTable must not be null");
            var databaseColumns = columns as DatabaseColumn[] ?? columns.ToArray();
            if (!databaseColumns.Any()) throw new ArgumentException("columns is empty", "columns");
            ...
            index.Columns.AddRange(databaseColumns);
            databaseTable.AddIndex(index);
