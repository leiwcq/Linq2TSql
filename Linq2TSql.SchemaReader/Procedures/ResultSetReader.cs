...
namespace Linq2TSql.SchemaReader.Procedures
{
    /// <summary>
    ...
    public class ResultSetReader
    {
        ...
        public void ExecuteProcedure(DatabaseStoredProcedure procedure)
        {
            ...
            if (_isOracle && procedure.Arguments.All(a => a.DatabaseDataType != "REF CURSOR"))
                return;
            ...
            using (var resultSet = new DataSet { Locale = CultureInfo.InvariantCulture })
            {
                using (DbConnection conn = _factory.CreateConnection())
                {
                    ...
                    conn.ConnectionString = _schema.ConnectionString;
                    using (var command = _factory.CreateCommand())
                    {
                        if (command == null) throw new NullReferenceException("命令为空");
                        ...
                        using (DbTransaction tx = conn.BeginTransaction())
                        {
                            ...
                            {
                                if (adapter == null) throw new NullReferenceException("数据适配器为空");
            ...
            foreach (DataTable table in resultSet.Tables)
            {
                ...
                foreach (DataColumn column in table.Columns)
                {
                    var dbColumn = new DatabaseColumn
                    {
                        ...
                        DbDataType = column.DataType.Name,
