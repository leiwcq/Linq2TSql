...
using Linq2TSql.SchemaReader.SqlGen;
...
namespace Linq2TSql.SchemaReader.Data
{
    ...
    public class ChangesWriter
    {
        ...
        {
            var value = string.Join(" ,", (from databaseColumn in _databaseTable.Columns
                where !databaseColumn.IsAutoNumber
                where _dataTable.Columns.Contains(databaseColumn.Name)
                where
                    IncludeAllFieldsInUpdate ||
                    !row[databaseColumn.Name].Equals(row[databaseColumn.Name, DataRowVersion.Original])
                let columnType = _columnTypes[databaseColumn.Name]
                let data = row[databaseColumn.Name]
                select
                    _sqlWriter.EscapedColumnName(databaseColumn.Name) + " = " +
                    _converter.Convert(columnType, data, databaseColumn.Name)).ToArray());
        ...
        {
            return String.Format(CultureInfo.InvariantCulture, _whereTemplate, (from pk in _unescapedKeyColumns
                let columnType = _columnTypes[pk]
                let data = row[pk, DataRowVersion.Original]
                select _converter.Convert(columnType, data, pk)).Cast<object>().ToArray());
        }
 ...
 VALUES ({0});";
        ...
        private static string FormattedColumns(string[] cols)
        {
            ...
            string sql = "  " + String.Join(JOIN_STRING, cols);
            return sql;
        }

        private string[] GetAllColumns()
        {
            return (from databaseColumn in _databaseTable.Columns
                where IncludeIdentity || !databaseColumn.IsAutoNumber
                select _sqlWriter.EscapedColumnName(databaseColumn.Name)).ToArray();
        }
