...
namespace Linq2TSql.SchemaReader.Compare
{
    class CompareViews
    {
        ...
        public void Execute(IEnumerable<DatabaseView> baseViews, IEnumerable<DatabaseView> compareViews)
        {
            ...
            var databaseViews = compareViews as DatabaseView[] ?? compareViews.ToArray();
            var views = baseViews as DatabaseView[] ?? baseViews.ToArray();
            foreach (var view in databaseViews)
            {
                var name = view.Name;
                var schema = view.SchemaOwner;
                var match = views.FirstOrDefault(t => t.Name == name && t.SchemaOwner == schema);
            ...
            foreach (var view in views)
            {
                var name = view.Name;
                var schema = view.SchemaOwner;
                var match = databaseViews.FirstOrDefault(t => t.Name == name && t.SchemaOwner == schema);
                if (match == null)
                {
