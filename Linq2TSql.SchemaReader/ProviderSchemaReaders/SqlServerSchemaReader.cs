...
ORDER BY cons.table_name, cons.constraint_name";
        ...
        private DataTable FindKeys(string tableName, string constraintType, DbConnection conn)
        {
            ...
            using (DbDataAdapter da = Factory.CreateDataAdapter())
            {
                ...
                if (type == null) throw new NullReferenceException("参数为空");
        ...
        {
            const string SQL_COMMAND = @"SELECT DISTINCT
cons.constraint_schema,
cons.constraint_name, 
...
ORDER BY o.name, c.name";
            ...
            return CommandForTable(tableName, connection, ComputedColumnsCollectionName, SQL_COMMAND);
        }
...
ORDER BY o.type;";
        ...
        protected override DataTable Triggers(string tableName, DbConnection conn)
        {
            ...
            const string SQL_COMMAND = @"SELECT SCHEMA_NAME(o1.uid) AS 'OWNER',
...
ORDER BY s.name, o.name";
            ...
            using (DbConnection conn = Factory.CreateConnection())
            {
                ...
                return CommandForTable(tableName, conn, DefaultConstraintCollectionName, SQL_COMMAND);
            }
        }
...
order by t.name, si.name ";
            return CommandForTable(tableName, connection, IndexesCollectionName, SQL_COMMAND);
        }
