...
namespace Linq2TSql.SchemaReader
{
    ...
    public class SqlWriter
    {
        ...
        {
            return (from column in _table.Columns
                where !column.IsAutoNumber && !column.IsTimestamp() && !column.IsComputed
        ...
        public IList<string> NonPrimaryKeyColumns
        {
            get
            {
                return (from column in _table.Columns
                    ...
                    where !PrimaryKeys.Contains(name)
                    where !column.IsTimestamp()
                    where !column.IsComputed
                    select name).ToList();
            }
        }
        ...
        public string SelectPageSql()
        {
            ...
            if (_sqlType == SqlType.MySql)
            {
                ...
                sb.AppendLine(" WHERE");
                sb.AppendLine("   rowNumber > (" + ParameterName("pageSize") + " * (" + ParameterName("currentPage") +
                              " - 1))");
...
FETCH NEXT @EndingRowNumber - @StartingRowNumber + 1 ROWS ONLY
        ...
        private string InsertSql(bool useOutputParameter, bool includeIdentityInInsert)
        {
            ...
            if (_table.HasAutoNumberColumn)
            {
                ...
                else if (_sqlType == SqlType.PostgreSql)
                {
                    ...
                    var seq = _table.Name + "_" +
                              ((_table.PrimaryKeyColumn != null) ? _table.PrimaryKeyColumn.Name : null) + "_seq";
        ...
        {
            return _table.Columns.Find(col => col.IsAutoNumber);
        }
        ...
        public string UpdateSql()
        {
            ...
            var cols = NonPrimaryKeyColumns.Select(name => EscapedName(name) + " = " + ParameterName(name)).ToList();
