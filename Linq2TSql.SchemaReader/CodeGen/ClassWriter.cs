...
using System.Linq;
using Linq2TSql.SchemaReader.CodeGen.CodeFirst;
using Linq2TSql.SchemaReader.DataSchema;

namespace Linq2TSql.SchemaReader.CodeGen
{
    /// <summary>
    ...
    public class ClassWriter
    {
        ...
        private void WriteManyToManyCollection(DatabaseTable foreignKey)
        {
            ...
            if (target == null)
            {
                Debug.WriteLine("Can't navigate the many to many relationship for " + _table.Name + " to " +
                                foreignKey.Name);
                return;
            }
        ...
        private void InitializeCollectionsInConstructor(string className)
        {
            ...
            using (_cb.BeginNest("public " + className + "()"))
            {
                foreach (var foreignKey in _table.ForeignKeyChildren)
                {
                    ...
                    {
                        var propertyName = _codeWriterSettings.Namer.ForeignKeyCollectionName(_table.Name, foreignKey,
                            fk);
        ...
        private void WriteCompositeKeyClass(string className)
        {
            ...
            using (
                _cb.BeginNest("public class " + className + "Key",
                    "Class representing " + _table.Name + " composite key"))
            {
