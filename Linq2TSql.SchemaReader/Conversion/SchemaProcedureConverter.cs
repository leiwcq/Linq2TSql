    ...
    class SchemaProcedureConverter
    {
        ...
        public void UpdateArguments(DatabaseSchema databaseSchema, DataTable arguments)
        {
            ...
            using (DataView sprocNames = new DataView(arguments))
            {
                ...
                sprocTable = !hasPackage
                    ? sprocNames.ToTable(true, argumentsKeyMap.SprocName, argumentsKeyMap.OwnerKey)
                    : sprocNames.ToTable(true, argumentsKeyMap.SprocName, argumentsKeyMap.OwnerKey,
                        argumentsKeyMap.PackageKey);
            }
        ...
        private static DatabaseStoredProcedure FindStoredProcedureOrFunction(DatabaseSchema databaseSchema, string name,
            string owner, string package)
        {
            var sproc = databaseSchema.StoredProcedures.Find(
                x => x.Name == name && x.SchemaOwner == owner && x.Package == package);
            if (sproc == null) //is it actually a function?
            {
                DatabaseFunction fun = databaseSchema.Functions.Find(
                    f => f.Name == name && f.SchemaOwner == owner && f.Package == package);
                if (fun != null)
                {
                    return fun;
                }
        ...
        {
            var existing = list.Find(arg => arg.Name == argName);
            if (existing == null)
            {
        ...
        public static List<DatabasePackage> Packages(DataTable dt)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                DatabasePackage package = new DatabasePackage
                {
                    ...
                    SchemaOwner = row[ownerKey].ToString()
                };
                list.Add(package);
            }
            return list;
